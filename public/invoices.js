...
$('#addInvoice').on('click', function (e) {
    ...
    if ($("#article").val()) {
        ...
        var rowId = $('#miTabla >tbody >tr').length + 1;
        ...
        var html =
            `<tr id="art-${rowId}">
                ...
                <td>${total_price}</td>
                <td>
                    <button class="btn btn-icon btn-light-danger delete-art" data-id="${rowId}">
        ...
        Swal.fire("Para continuar", "Elija un articulo", "warning");
    }
...
$(document).delegate('button.delete-art', 'click', function(e) {
    e.preventDefault();
    var id = $(this).data('id');
